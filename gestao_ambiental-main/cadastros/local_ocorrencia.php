...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($qual == 'local') {
        ...
        if ($carregarDados == 'nao') {
            ...
            if (!empty($id)) {
                $sql = "UPDATE local SET local = ? WHERE id = ?";
                $stmt = $db->query($sql, [$local, $id], 'si');

                if ($stmt && $db->affectedRows() >= 0) {
                    echo json_encode(["status" => "true", "message" => "Local atualizado com sucesso."]);
                } else {
                    echo json_encode(["status" => "false", "message" => "Erro ao atualizar local."]);
                }
                exit;
            } else {
                $sql = "INSERT INTO local (local) VALUES (?)";
                $stmt = $db->query($sql, [$local], 's');

                if ($stmt && $db->affectedRows() > 0) {
                    echo json_encode(["status" => "true", "message" => "Local adicionado com sucesso."]);
                } else {
                    echo json_encode(["status" => "false", "message" => "Erro ao adicionar local."]);
                }
                exit;
            }
        }
        ...
        if ($carregarDados == 'delete') {
            ...
            if ($stmt && $db->affectedRows() > 0) {
                echo json_encode(["dados" => "Local deletado com sucesso."]);
            } else {
                echo json_encode(["error" => "Erro ao deletar local."]);
            }
            exit;
        }
    }
    ...
    if ($qual == 'ocorrencia') {
        ...
        if ($carregarDados == 'nao') {
            ...
            if (empty(trim($ocorrencia))) {
                echo json_encode(["error" => "Ocorrência é obrigatória."]);
                exit;
            }

            if (!empty($id)) {
                $sql = "UPDATE ocorrencia SET ocorrencia = ? WHERE id = ?";
                $stmt = $db->query($sql, [$ocorrencia, $id], 'si');

                if ($stmt && $db->affectedRows() >= 0) {
                    echo json_encode(["status" => "true", "message" => "Ocorrência atualizada com sucesso."]);
                } else {
                    echo json_encode(["status" => "false", "message" => "Erro ao atualizar ocorrência."]);
                }
                exit;
            } else {
                $sql = "INSERT INTO ocorrencia (ocorrencia) VALUES (?)";
                $stmt = $db->query($sql, [$ocorrencia], 's');

                if ($stmt && $db->affectedRows() > 0) {
                    echo json_encode(["status" => "true", "message" => "Ocorrência adicionada com sucesso."]);
                } else {
                    echo json_encode(["status" => "false", "message" => "Erro ao adicionar ocorrência."]);
                }
                exit;
            }
        }
        ...
        if ($carregarDados == 'delete') {
            ...
            if ($stmt && $db->affectedRows() > 0) {
                ...
                echo json_encode(["error" => "Erro ao deletar ocorrência."]);
            }
            exit;
        }
    }
